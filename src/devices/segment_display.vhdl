...
begin
    mode_reg: entity work.MemoryMappedReg generic map (1, x"5100")    port map (clk, iow, addr, data_in(0) => data(0), data_out(0) => display_hex);
    low_reg:  entity work.MemoryMappedReg generic map (size, x"5104") port map (clk, iow, addr, data, data_low);
    high_reg: entity work.MemoryMappedReg generic map (size, x"5108") port map (clk, iow, addr, data, data_high);
